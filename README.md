...
| `representation_gap.py` | Crosses VMP shelf presence (region × style, from the launch history) against a **curated, sourced stature reference** (DOCG/DOCa status, canonical-variety standing) → ranked **candidate** gaps where high-stature categories are thinly carried (e.g. Aglianico). Writes `representation_gap.json` (embedded → the app's *Marked → Gap* panel). A hypothesis generator, heavily labelled: presence is a partial sample, and the strongest signals (peer Nordic monopolies, real sell-through) are marked *pending* |
